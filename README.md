...
# Train models locally (downloads data, ~8-12 min one-time)
pip install -r requirements-train.txt
# Use fewer playlists for a faster/lighter run:  python train.py --slices 30
python train.py

# Launch the app (serving deps only)
pip install -r requirements.txt
streamlit run app.py
```

> Don't want to train? The app auto-downloads pre-trained models from a GitHub
> Release on first launch (see `model_store.py`) — just `pip install -r requirements.txt && streamlit run app.py`.
...
The app is deploy-ready. The trained `models/` folder is **not** in the repo (it's
gitignored); instead the app downloads a ~41 MB bundle from a GitHub Release on
first boot (see `model_store.py`). Serving uses a light `requirements.txt` (no
`implicit`/`scipy`), so it fits the free 1 GB tier.

1. Push this repo to GitHub (already done).
2. At [share.streamlit.io](https://share.streamlit.io), create an app from the repo with **main file path** `spotify-recommender/app.py`.
3. In **Advanced settings → Secrets**, add (TOML):
   ```toml
   SPOTIFY_CLIENT_ID = "your_client_id"
   SPOTIFY_CLIENT_SECRET = "your_client_secret"
   ```
   Cover art/links need these; the recommender itself runs without them.
4. Deploy. First boot downloads the model bundle (~10 s), then it's cached.

> Updating models? Re-run `train.py`, re-pack `tar czf models.tar.gz -C spotify-recommender models`, and upload it to a new GitHub Release; point `MODELS_URL` (env/secret) at the new asset.
...
├── model_store.py       # Downloads model bundle from GitHub Release on first boot
├── app.py               # Streamlit UI
├── requirements.txt       # Serving deps (deployed)
├── requirements-train.txt # Adds implicit/scipy/sklearn for training
├── .env.example
└── models/              # Generated by train.py / downloaded on deploy (gitignored)
```
